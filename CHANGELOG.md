...
- **Backend dependencies** — Updated backend libraries: Ktor 2.3.6 → 2.3.13, Kotlin 1.9.20 → 1.9.25, Logback 1.4.11 → 1.5.38, PostgreSQL driver 42.7.1 → 42.7.13, Micrometer 1.11.0 → 1.17.0, Testcontainers 1.21.3 → 1.21.4, JUnit Jupiter 5.10.1 → 5.14.4, and mockito-kotlin 5.2.1 → 5.4.0
- **Build toolchain (Gradle)** — Upgraded the Gradle wrapper from 7.6.4 to 8.7, clearing the "incompatible with Gradle 8.0" deprecation warnings; added an explicit `junit-platform-launcher` so the newer JUnit 5.14 platform runs under Gradle. Micrometer was upgraded via its legacy `micrometer-registry-prometheus-simpleclient` module, which keeps the existing metric names (the modern client strips reserved suffixes like `_total` from gauges and would have broken the Grafana dashboards)
